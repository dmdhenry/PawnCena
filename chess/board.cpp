...
// TODO: Clean this function up! Way too long!
...
bool Board::is_legal_move(const Move& move, Color player) {
    ...
    std::string notation = move.get_move();
    if (notation.length() != 4 && notation.length() != 6 && notation != "oo" && notation != "ooo") {
        return false;
    }
    ...
    switch (piece) {
        ...
        case BLACK_PAWN: {
            ...
            int back_rank = (piece == WHITE_PAWN) ? 7 : 0;
